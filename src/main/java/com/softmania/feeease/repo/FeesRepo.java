...
public interface FeesRepo extends JpaRepository<Fees, Integer> {
    ...
    List<Fees> getFeesPaidByYear(@Param("year") int year, @Param("schoolId") int schoolId);

    @Query("SELECT f FROM Fees f JOIN Students s ON f.student.id = s.id " +
            "WHERE f.depositMonthYear = :depositMonthYear AND s.school.id = :schoolId")
    List<Fees> getFeesPaidByMonthYear(@Param("schoolId") int schoolId, @Param("depositMonthYear") String depositMonthYear);

    @Query("SELECT f FROM Fees f JOIN Students s ON f.student.id = s.id " +
            "WHERE f.student.id = :studentId AND s.school.id = :schoolId")
    List<Fees> getFeesPaidByStudent(@Param("schoolId")int schoolId, @Param("studentId")int studentId);
}
